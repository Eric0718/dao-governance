...
const setupContracts: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const cryaToken = await ethers.getContract("CryaToken", deployer)
  const timeLock = await ethers.getContract("TimeLock", deployer)
  const governor = await ethers.getContract("CryaGovernor", deployer)
...
setupContracts.tags = ["all", "setupgovernance"]